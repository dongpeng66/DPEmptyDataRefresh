    ...
    if (([self dzn_shouldDisplay] && [self dzn_itemsCount] == 0) || [self dzn_shouldBeForcedToDisplay])
    {
        ...
        if (customView) {
            ...
            if (buttonImage) {
                ...
                view.button.layer.borderColor=[UIColor redColor].CGColor;
...
- (void)setupConstraints
{
    ...
    if (_customView) {
        ...
        if ([self canShowButton]) {
            NSDictionary *me = @{@"padding": @((CGRectGetWidth([UIScreen mainScreen].bounds)-150)/2)};
            [subviewStrings addObject:@"button"];
            views[[subviewStrings lastObject]] = _button;
            
            [self.contentView addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|-(padding@750)-[button(>=150)]-(padding@150)-|"
                                                                                     options:0 metrics:me views:views]];
        }
        // or removes from its superview
        else {
